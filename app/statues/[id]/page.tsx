// StatueDetailPage avec logique notFound + chargement
"use client";
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if ((!Array.isArray(dataSkills) || dataSkills.length === 0) && (!Array.isArray(dataCharacters) || dataCharacters.length === 0)) {
          setNotFound(true);
        } else {
          setNotFound(false);
        }
  ...
  if (notFound) {
    return (
      <div className="min-h-screen flex items-center justify-center text-center text-white p-6">
        <p className="text-lg">Cette statue n’est pas disponible dans la base de données sélectionnée.</p>
      </div>
    );
  }
