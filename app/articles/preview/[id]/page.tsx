...
async function getArticle(id: string): Promise<Article | null> {
  const res = await fetch(`${API_URL}/items/Articles/${id}?fields=*,user_created.first_name,likes,status`, {
    cache: "no-store",
  });
...
async function getRandomArticles(currentId: string): Promise<Article[]> {
  const res = await fetch(`${API_URL}/items/Articles?limit=100&fields=id,title,date_created,images`, {
    cache: "no-store",
  });
