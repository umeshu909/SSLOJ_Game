...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const response = await fetch(`${CMS_URL}/items/Articles?fields=id,title,text,date_created,images,user_created.first_name,status,slug&sort=-date_created&limit=${limit}&offset=${offset}&filter[status][_eq]=published&meta=filter_count`);
