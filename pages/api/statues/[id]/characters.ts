...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const sqlFilePath = path.join(process.cwd(), "sql", "getStatuesCharacters.sql");
        ...
        if (!data) {
            return res.status(404).json({ error: "DPersonnages de la statue non trouvé" });
        }
        ...
        res.status(200).json(cleanData); // Renvoi des statues formatés
    } catch (error) {
        console.error("Erreur lors de la récupération des  personnages des statues:", error);
