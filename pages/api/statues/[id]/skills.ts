...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const sqlFilePath = path.join(process.cwd(), "sql", "getStatuesSkills.sql");
        ...
        if (!data) {
            return res.status(404).json({ error: "Skills de la statue non trouvé" });
        }
        ...
        console.error("Erreur lors de la récupération des  skills des statues:", error);
