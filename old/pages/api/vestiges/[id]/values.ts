...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    const { id } = req.query;
    const level = req.query.level ? Math.floor(parseInt(req.query.level as string, 10) / 5) : 1;
    ...
    try {
        ...
        const sqlFilePath = path.join(process.cwd(), "sql", "getVestigesValues.sql");
        ...
        const data = await db.all(sqlQuery, [id, level]);
