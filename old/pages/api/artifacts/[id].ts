...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const sqlFilePath = path.join(process.cwd(), "sql", "getArtifactsDetail.sql");
        ...
        const data = await db.get(sqlQuery, [id, level]);

        if (!data) {
            return res.status(404).json({ error: "Détail de l'artefact non trouvé" });
        }
